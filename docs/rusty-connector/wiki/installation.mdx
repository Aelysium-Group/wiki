...
:::info
You must have working Redis and MariaDB databases for RustyConnector to opperate!
:::
...
Anytime an MCLoader server boots it will automatically attempt to establish a Magic Link connection with the Proxy. If the connection is successful, the MCLoader will be registered into one of the predefined server families. (You can read up on Families [here](./concepts/family.md))
...
:::note
You can read more about Families [here](./concepts/family.md). Make sure you edit `root-family.name` to match whichever family should be your default.
:::
